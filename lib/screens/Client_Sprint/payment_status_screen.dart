...
import '../../widgets/Client_Sprint/payment_list.dart';
...
class _PaymentStatusScreenState extends State<PaymentStatusScreen> {
    ...
    try {
      if (Provider.of<Auth>(context, listen: false).token != null) {
        await Provider.of<Sold>(context, listen: false)
            .fetchAndSetPaymentStatus(pickedDates);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    final sold = Provider.of<Sold>(context, listen: false);
                ...
                Printing.layoutPdf(onLayout: (PdfPageFormat format) async {
                  try {
                    return await generateInvoice(
                      PdfPageFormat.a4,
                      sold.paymentStatusList,
                      sold.firstPickedDate,
                      sold.lastPickedDate,
                      Sold.tableHeaders,
                      auth.denomination,
                      'Etat de paiements',
        ...
        body: FutureBuilder(
          ...
          builder: (ctx, snapShot) =>
              snapShot.connectionState == ConnectionState.waiting
                  ...
                  : PaymentList(size, changeFirstState, changeLastState,
                      restart),
        ),
      ),
    );
